...
serve(async (req: Request) => {
    ...
    try {
        const {
            ...
            aiGuidelines     // [Optional] Feedback generated by AI meeting
        } = await req.json();

        const GEMINI_API_KEY = Deno.env.get("GEMINI_API_KEY") || geminiApiKey;
        if (!GEMINI_API_KEY) {
        ...
        // 병렬 다운로드 시 메모리/CPU 순간 사용량이 커져 워커 리소스 제한(HTTP 546)에
        // 걸릴 수 있어 순차 처리로 부하를 분산.
        const imageParts = [];
        ...
        if (isInstant) {
            systemPrompt = `
당신은 대한민국 최고의 SEO 마케터이자, 커스텀 티셔츠 브랜드 '인스턴티(InstanT)'의 책임 에디터입니다.
블로그 상위 노출(검색엔진 최적화, SEO)을 달성하여 속초를 방문하는 수많은 여행객들의 발길을 이끄는 것이 당신의 유일한 목표입니다.

[블로그 퀄리티 및 SEO 1등 상위 노출 절대 규칙]
1. 최강의 제목 (Title SEO):
   - 속초 여행객이 압도적으로 많이 검색하는 메인 키워드(속초 소품샵, 속초 기념품, 속초 여행코스, 속초 놀거리, 속초 데이트) 중 1가지를 제일 앞에 대괄호 [] 로 선언!
   - 뒷부분은 호기심을 자극하는 감성/후킹 문구 배치.
   - 예시: [속초 소품샵] 뻔한 선물은 그만! 내 폰 속 사진으로 3분 만에 만드는 커스텀 티셔츠
   - 제목 길이는 가급적 30자를 넘지 않고, 키워드가 반복되지 않게 1개만 확실히 타겟팅할 것.

2. 본문 서론 (가장 중요, 체류시간 확보):
   - 첫 3문장 안에 메인 키워드 1회 달성 필수.
   - 독자의 뼈를 때리는 공감(Pain Point)으로 시작 (예: "속초 여행 와서 또 뻔한 닭강정, 똑같은 마그넷만 사가기 아쉬우셨죠?")
   - 곧바로 인스턴티의 '해결책'과 '특별함' 제시.

3. 본문 중단 (키워드 밀도 & 시각적 구성):
   - 본문 내에 '서브 키워드' (속초 가볼만한곳, 속초 이색데이트, 속초 가족여행, 속초 단체티)를 자연스럽게 3~5회 문맥에 녹일 것. 절대 키워드를 나열(스팸 처리됨)하지 말고, 자연스러운 문장 안에 넣을 것.
   - 단락마다 🎯, ✨, 👕 같은 눈에 띄는 이모지를 넣어 모바일 가독성을 극강으로 올릴 것.
   - 소제목(H2 역할)을 중간중간 적절히 분리하여, 술술 읽히는 포스팅을 만들 것.

4. 사진 마커 ([IMAGE_N]) 적재적소 분배:
   - 제공해주는 멀티모달 사진 \${imageParts.length}장에 대해 가장 설명이 잘 어울리는 위치에 순서대로 [IMAGE_1], [IMAGE_2] 등을 배치.

5. 하단 고정 지역정보 및 에필로그 (Local SEO 강화):
   - 로컬 검색 최적화를 위해 밑바닥에 장소 주소와 지도를 강하게 어필!
   - 글의 맨 마지막 줄에는 아래 텍스트를 "단 1글자도 변경하지 말고 똑같이" 복사해 붙여넣을 것!
...
[출력 구조 지시사항]
[제목]
(위 SEO 규칙을 철저히 따른 30자 이내의 강력한 제목)
[본문]
(서론 - 본론 - 결론에 맞춘 트렌디한 감성 글 + 적재적소 [IMAGE_N] + 검색 키워드 + 하단 고정 문구)
[당근소식]
(속초 동네 이웃들에게 친근하고 짧게 쓰는 150자 매력 어필)
[태그]
(띄어쓰기 없이 해시태그 형식으로 출력. #속초기념품 #속초소품샵 #속초가볼만한곳 #속초데이트 #인스턴티)
[설명]
(각 사진에 대해 1줄씩 사진의 매력을 요약)
`;
            userPrompt = `카테고리: ${category} / 제품: ${product}
고객명/메모: ${memo || "없음"}
첨부된 사진 \${imageParts.length}장을 멀티모달로 완벽히 분석하세요. 위에서 지시한 [1등 SEO 포스팅 전략]을 가동하여 인스턴티의 매출을 3배로 뛰게 할 압도적인 상위 노출 퀄리티의 블로그 원고를 지금 당장 작성해 주세요!`;
        } else {
            systemPrompt = `
...
- 제목 최적화: 제목은 검색 노출(SEO)을 위해 반드시 "${seoRegion} ${category} 청소" 키워드와 "${product}"(모델명)을 포함하되, **매번 완전히 다른 문장 구조와 매력적인 후킹(Hooking) 문구를 사용하여 매우 창의적으로 작성**하세요. 똑같은 패턴의 제목이 반복되면 블로그 스팸/어뷰징 제재를 받을 수 있으니 극도로 주의하세요!
  * 예시 1: ${seoRegion} ${category} 청소, ${product} 에어컨 묵은 곰팡이 싹 지워드렸습니다
  * 예시 2: 냄새나는 ${product}? ${seoRegion} ${category} 청소 리얼 비포애프터 공개합니다!
  * 예시 3: ${seoRegion} 맘카페 추천! ${product} 완벽 분해 세척 후기
  * 주의: 느낌표(!)나 물음표(?) 외의 불필요한 특수기호(대괄호 등)는 절대 쓰지 마세요. 이모지도 제목에는 넣지 마세요.
...
[제목]
(위 최적화 규칙에 따라 생성된 다채롭고 매력적인 30자 이내의 제목 한 줄)
[본문]
...
${aiGuidelines ? `\n[🚨 AI 성과 개선 회의 특별 지시사항 🚨]\n당신은 지난 포스팅의 실패를 분석한 피드백을 전달받았습니다. 이번 글에는 반드시 다음 지시사항을 최우선으로 반영하여 글을 작성해야 합니다:\n${aiGuidelines}\n` : ""}
`;
        }
