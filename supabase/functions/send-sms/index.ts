    ...
    try {
        ...
        if ((payload.type === 'INSERT' && payload.table === 'bookings' && payload.record) || 
            ...
            if (!apiKey || !apiSecret || !senderPhone) {
                console.warn('[Webhook:INSERT] Missing credentials or sender phone. Skipping all SMS.', { hasApiKey: !!apiKey, hasApiSecret: !!apiSecret, senderPhone });
                return new Response(JSON.stringify({ error: 'Missing credentials' }), { status: 200, headers: corsHeaders });
            }

            // A. 고객 확정 문자 발송 (설정 확인)
            if (business?.auto_confirm_sms && phone && phone.length >= 10) {
                ...
                console.log(`[Webhook:INSERT] Attempting customer SMS to ${phone}`);
                try {
                    await sendSms(apiKey, apiSecret, senderPhone, phone, text)
                    await supabase.from('bookings').update({ sms_sent_initial: true }).eq('id', id)
                } catch (err: any) {
                    console.error(`[Webhook:INSERT] Customer SMS failed: ${err.message}`);
                }
            } else {
                console.log(`[Webhook:INSERT] Customer SMS skipped. (Enabled: ${!!business?.auto_confirm_sms}, Phone: ${!!phone})`);
            }

            // B. 파트너 알림 문자 발송 (설정 확인 및 본인 제외 동일 업체 파트너들에게)
            if (business?.auto_partner_sms !== false) { // null이거나 true일 때 발송
                try {
                    const { data: partners } = await supabase
                        .from('profiles')
                        .select('solapi_from_number, sender_number, nickname')
                        .eq('business_id', business_id)
                        .neq('id', user_id);

                    if (partners && partners.length > 0) {
                        const partnerMsg = `[클린브로] 새 예약 알림: ${dateTimeStr}, ${customer_name || '고객'}님 (${product || '청소'}) - 앱에서 확인해 주세요.`
                        console.log(`[Webhook:INSERT] Found ${partners.length} partners to notify.`);

                        for (const partner of partners) {
                            const targetPhone = partner.solapi_from_number || partner.sender_number;
                            if (targetPhone) {
                                try {
                                    console.log(`[Webhook:INSERT] Notifying partner ${partner.nickname || 'Unknown'} at ${targetPhone}`);
                                    await sendSms(apiKey, apiSecret, senderPhone, targetPhone, partnerMsg);
                                } catch (pErr: any) {
                                    console.error(`[Webhook:INSERT] Partner notification failed for ${targetPhone}: ${pErr.message}`);
                                }
                            }
                        }
                    } else {
                        console.log('[Webhook:INSERT] No other partners found to notify.');
                    }
                } catch (pSearchErr: any) {
                    console.error(`[Webhook:INSERT] Partner search failed: ${pSearchErr.message}`);
                }
            } else {
                console.log('[Webhook:INSERT] Partner notification skipped (Disabled in settings).');
            }

            return new Response(JSON.stringify({ message: 'Insert Webhook processed' }), {
                status: 200,
                headers: { ...corsHeaders, 'Content-Type': 'application/json' }
            })
        }
